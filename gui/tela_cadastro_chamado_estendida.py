...
def acicionar_chamado():
    if TITULO.get() == "" or DESCRICAO.get() == "" or URGENCIA.get() == "" or RESPONSAVEL.get() == "" or USUARIO.get() == "" or SENHA.get() == "":
        txt_resultado.config(text="Você deve informar todos os campos", fg="red")
    else:
...
def listar_chamados():
    ...
    inicializar_base_dados()
    cursor.execute("SELECT * FROM `chamado` ORDER BY `titulo` ASC")
    fetch = cursor.fetchall()
    for data in fetch:
        tabela.insert('', 'end', values=(data[1], data[2], data[3], data[4], data[5], data[6]))
    cursor.close()
    conexao.close()
    txt_resultado.config(text="Chamados lidos com sucesso", fg="black")
...
def sair():
    result = tkMessageBox.askquestion('Você realmente quer sair?', icon="info")
    if result == 'yes':
...
alta = Radiobutton(opcoes, text="Alta", variable=URGENCIA, value="A", font=('arial', 16)).pack(side=LEFT)
baixa = Radiobutton(opcoes, text="Baixa", variable=URGENCIA, value="B", font=('arial', 16)).pack(side=LEFT)

# ==================================NOMES=======================================
txt_nome_tela = Label(topo, width=900, font=('arial', 24), text="Registro de Chamados")
txt_nome_tela.pack()
txt_titulo = Label(formularios, text="Título:", font=('arial', 16), bd=15)
txt_titulo.grid(row=0, stick="e")
txt_descricao = Label(formularios, text="Descrição:", font=('arial', 16), bd=15)
...
txt_responsavel = Label(formularios, text="Responsável:", font=('arial', 16), bd=15)
...
# ==================================BOTÕES=====================================
btn_criar = Button(botoes, width=10, text="Cadastrar", command=acicionar_chamado)
